...
# from t_smiles.Tools.DataConv import BCDataConv # TODO: I hope this is not important
...
class SeqUtils:
    ...
    def Seq2IntTensor(self, seqs, tokens, pad=False, flip=False, max_length=200):
        # items of input seqs are in the same length
        # input:[[Brc1ccccc1Cc1cccs1],[CCCCC(C)c1cccs1]]
        # output:[[21 23 1 ....5 8 9],[5 5..........4 3 8]]
        if pad:
    ...
    def read_object_property_file(
        ...
        **kwargs,
        ...
        data = []
        if (
            len(cols_to_read) == 1 and cols_to_read[0] == -1
        ):  # read all data as one list
    ...
    @staticmethod
    def load_h5f(filename: str, split=True):
        h5f = h5py.File(filename, "r")

        if split:
            data_train = h5f["data_train"][
                :
            ]  # +		shape	(40000, 120, 33)	tuple
        else:
            data_train = None
    ...
    def load_smiles_data(
        ...
        save_dir = os.path.join(
            data_dir, filename.split(".")[0] + f"_data_dict{suffix}.joblib"
        )
        trans_save_dir = os.path.join(
            data_dir, filename.split(".")[0] + f"_transformer{suffix}.joblib"
        )
        ...
        dataframe = pd.read_csv(
            file, header=header, index_col=index_col, delimiter=delimiter
        )
        ...
        if cv:
            log(
                f"Splitting data into {k} folds. Each fold has train, val, and test sets."
            )
            ...
            for i, (train_idx, test_idx) in enumerate(cv_split.split(X, y)):
                ...
                if create_val:
                    x_val, x_test, y_val, y_test = train_test_split(
                        X[test_idx], y[test_idx], test_size=0.5, random_state=seed
                    )
                    data_dict[f"fold_{i}"] = {
...
class Test:
    ...
    def test_Seq2IntTensor():
        ...
        res, tks = SeqUtils.Seq2IntTensor(
            seqs, token, pad=True, max_length=100, pad_symbol=" ", flip=False
        )
    ...
    def test_read_object_property_file():
        print(
            "---------------test_read_object_property_file----------------------------------"
        )
        file = r"D:\ProjectTF\BioChemoTCH\DataSet\ChEMBL\chembl_10.smi"
        smiles = SeqUtils.read_object_property_file(file, "\t", [0])
