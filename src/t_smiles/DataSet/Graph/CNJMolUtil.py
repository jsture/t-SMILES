...
from t_smiles.MolUtils.RDKUtils.RDKEnumerator import RDKEnumerateSmiles
...
class CNJMolUtil:
    ...
    def combine_ex_smiles(bfs_ex_smiles, delimiter="^"):
        ...
        for i in range(1, nlen):
            if bfs_ex_smiles[i - 1] != "&" and bfs_ex_smiles[i] != "&":
                sml += split_char
                skeleton += split_char
    ...
    def aug_ex_smiles(
        ...
        for frag in bfs_ex_smiles:
            if frag == "&" and frag == "^":
                ...
                aug_frags = RDKEnumerateSmiles.enumerate_smiles(
                    frag, n_aug, code, isomericSmiles, kekuleSmiles
                )
