...
from t_smiles.Tools.GraphTools import GTools
from t_smiles.Tools.MathUtils import BCMathUtils
from t_smiles.DataSet.STDTokens import CTokens
from t_smiles.DataSet.JTNN.MolTree import Vocab, MolTree
from t_smiles.DataSet.STDTokens import STDTokens_Frag_File
from t_smiles.DataSet.Graph.CNJMolUtil import CNJMolUtil
from t_smiles.DataSet.Graph.CNXBinaryTree import CNXBinaryTree
...
class CNJTMolTree(MolTree):
    def __init__(
        ...
        if smiles is not None and len(smiles) > 0:
            self.init_from_smile(
                smiles, jtvoc, ctoken, dec_alg=dec_alg, kekuleSmiles=kekuleSmiles
            )
        else:
            self.mol = None
    ...
    def init_from_smile(
        ...
        try:
            ...
            super(CNJTMolTree, self).__init__(
                smiles=smiles,
                dec_alg=dec_alg,
                kekuleSmiles=kekuleSmiles,
                frg_random=frg_random,
            )
            ...
            (
                ...
                self.ctoken, self.bfs_binary_tree, extra_dummy=True
            ...
            if self.bfs_ex_vocids is None:
                self.mol = None
            self.combine_ex_smiles, self.skeleton = CNJMolUtil.combine_ex_smiles(
                self.bfs_ex_smiles
            )
    ...
    def create_bfs_binary_tree(self, show=False):  #
        ...
        idx = 0
        nx_node = self.graph_nx.nodes[idx]
        self.bfs_binary_tree = CNJTMolTreeNode(
            idx=idx, nx_node=nx_node
        )  # the first node
        self.bfs_node_map[idx] = self.bfs_binary_tree
        visited[idx] = 1
        ...
        for node in self.nx_binarytree.nodes:
            ...
            if isinstance(idx, CNJTMolTreeNode):
                ...
                self.nx_binarytree.nodes[idx]["smile"] = self.bfs_node_map[idx].data[
                    "smile"
                ]
        ...
        try:
            while len(queue) != 0:
                ...
                if item is not None:
                    ...
                    bfs_ex_vocid.append(
                        item.data["idx_voc"] if "idx_voc" in item.data else -1
                    )
        ...
        try:
            for i in range(len(bfs_ex_smiles)):
                if not CNJMolUtil.is_dummy(bfs_ex_smiles[i]):
                    bfs_ex_smiles[i] = CNJMolUtil.valid_smiles(
                        bfs_ex_smiles[i], ctoken=ctoken
                    )

            if (
                nlen > 1 and bfs_ex_smiles[1] == ctoken.invalid_token
            ):  # jump this dummy one
                bfs_ex_smiles.pop(1)
    ...
    def bfs_ex_reconstruct_amt(
        ctoken, bfs_ex_id=None, bfs_ex_smiles=None, clean_up=True, show=False
    ):
        g, dummy_nodes, bfs_ex_smiles, bfs_ex_id = CNJTMolTree.bfs_recon_validate(
            ctoken, bfs_ex_id=bfs_ex_id, bfs_ex_smiles=bfs_ex_smiles, show=show
        )
        ...
        try:
            ...
            g = CNJTMolTree.bfs_g_reorder(
                g
            )  # this is time consuming process, it could be ignored
            if show:
                GTools.show_network_g_cnjtmol(g)
        except Exception as e:
    ...
    def bfs_ex_reconstruct(
        ...
        id_map_rvs = {
            ...
            for key, val in sorted(
                id_map.items(), key=lambda ele: ele[0], reverse=False
            )
        }
    ...
    def get_nb_map(g):
        ...
        neighbor_map = (
            [()] * n_nodes
        )  # self.neighbor_map	[{1}, {0, 2}, {1, 3, 5}, {2, 4}, {3}, {2, 6}, {5}]	list
...
class CNJMolUtils:
    def encode_single(self, smls: str, ctoken, dec_alg):
        # print('[smls is]:', smls)
        amt_bfs_smarts = ""
        try:
            ...
            for sml in sub_smils:
                ...
                s, skeleton = CNJMolUtil.combine_ex_smiles(
                    bfs_ex_smiles
                )  # # noqa: F841
...
def preprocess():
    ...
    # kekuleSmiles = False
    kekuleSmiles = True  # noqa: F841

    ctoken = CTokens(
        STDTokens_Frag_File(None), max_length=256, invalid=True, onehot=False
    )
    ...
    print(smlfile)
    df = pd.read_csv(
        smlfile, delimiter=",", header=None, skip_blank_lines=True
    ).squeeze("columns")
            ...
            try:
                ...
                for i, sub_s in enumerate(sub_smils):
                    ...
                    if cnjtmol.mol is not None:
                        ...
                        joined_smiles, skeleton = CNJMolUtil.combine_ex_smiles(
                            cnjtmol.bfs_ex_smiles
                        )
                        joined_smarts, _ = CNJMolUtil.combine_ex_smiles(
                            cnjtmol.bfs_ex_smarts
                        )
