...
from t_smiles.DataSet.Graph.CNJTMol import CNJTMolTreeNode, CNJTMolTree
from t_smiles.DataSet.Graph.CNJMolUtil import CNJMolUtil
from t_smiles.DataSet.JTNN.MolTree import MolTreeNode, MolTreeUtils
from t_smiles.DataSet.JTNN.ChemUtils import ChemUtils
from t_smiles.MolUtils.RDKUtils.Frag.RDKFragUtil import RDKFragUtil, CODE_Alg
from t_smiles.MolUtils.RDKUtils.RDKAssembling import RDKAssembling
from t_smiles.MolUtils.RDKUtils.Utils import RDKUtils
...
class CNJMolAssembler:
    ...
    def get_hparam(self, asm_alg="CALG_TSDY"):
        ...
        if asm_alg == "CALG_TSDY":  #'Dummy'
            ...
            hparam["n_candidates"] = (
                5  # get more candidates for Goal-directed Reconstruction
            )
    ...
    def enum_assemble(
        node, neighbors, prev_nodes=[], prev_amap=[], hparam=CNJASM_HParam()
    ):
        candidates = []
        candidates_sml = []
        try:
            if hparam["alg"] == "CALD_TSSA":
                candidates = ChemUtils.enum_assemble(
                    node, neighbors, prev_nodes, prev_amap
                )
                ...
                hparam["alg"] == "CALG_TSDY"
                or hparam["alg"] == "CALG_TSID"
                or hparam["alg"] == "CALG_TSIS"
            ):
                frags = [node.smiles]
                ...
                for nb in neighbors:
                    ...
                    for ncd in nb_cands:
                        ...
                        new_mol_cands, scores = RDKAssembling.assemb_mols_dummy(
                            base_mol,
                            nb_mol,
            ...
            print(
                "[CNJMolAssembler.enum_assemble].Exception.neighbors",
                neighbors[0].smiles,
            )
    ...
    def Convert_to_JTMoltree(
        ...
        bfs_ex_nodeid, bfs_ex_vocids, bfs_ex_smiles, new_vocs, bfs_ex_smarts = (
            CNJTMolTree.get_bfs_ex(ctoken, bfs_binary_tree, extra_dummy=False)
        )

        bfsnode = bfs_binary_tree
        bfsnode.data["idx_voc"]
        ...
        g.add_node(
            root.idx, nid=root.idx, smile=root.smiles, jtnode=root, smarts=root.smarts
        )
        try:
            while len(bfs_queue) > 0 and not stop:
                ...
                if node_left is not None and not CNJMolUtil.is_dummy(
                    node_left.data["smile"]
                ):
                    idx = node_left.idx
                    node_left.data["idx_voc"]
                    smile = node_left.data["smile"]
                    smarts = node_left.data["smarts"]
                    wid = ctoken.STDTokens.vocab.get_index(smile)

                    slots = MolTreeUtils.get_slots(smile)
                    MolTreeUtils.get_slots(node_x.smiles)
                ...
                if node_right is not None and not CNJMolUtil.is_dummy(
                    node_right.data["smile"]
                ):
                    if node_x in parent_map:
                        node_x = parent_map[node_x]

                    idx = node_right.idx
                    node_right.data["idx_voc"]
    ...
    def get_target_candidates(candidates, n_candidates, alg="random"):
        if len(candidates) < n_candidates:
            ...
            print(
                f"[len of candidates]:{len(candidates)}, select the first {n_candidates} as target"
            )
            ...
            if alg == "atomenv":
                ...
                gfun = ModelUtils.str_to_class(
                    module_name=module_name, class_name=gfun_name
                )
                if gfun is not None:
                    gfun = gfun()
    ...
    def assemble_JTMolTree_leaf_first(
        moltree,
        uvisited,
        hparam=CNJASM_HParam(),
    ):
            ...
            for node in node_leaf:
                ...
                if len(neib) > 0:
                    ...
                    if hparam["candidate_mode"] == "random_one":
                        candidates = CNJMolAssembler.enum_assemble(
                            node, neib, prev_nodes=[], prev_amap=[], alg=alg
                        )
                    else:  # candidate_tree
                        if len(node.candidates) == 0:
                            candidates = CNJMolAssembler.enum_assemble(
                                node, neib, prev_nodes=[], prev_amap=[], hparam=hparam
                            )
                        else:
                            can_smls = []
                            for cand in node.candidates:
                                ...
                                if len(neib[0].candidates) == 0:
                                    cands = CNJMolAssembler.enum_assemble(
                                        node_copy,
                                        neib,
                                        prev_nodes=[],
                                        prev_amap=[],
                    ...
                    if len(candidates) > n_candidates:
                        candidates = CNJMolAssembler.get_target_candidates(
                            candidates,
                            n_candidates,
                        ...
                        candidates = sorted(
                            candidates, reverse=True, key=lambda x: x[3]
                        )
    ...
    def assemble_one_node(
        ...
        try:
            ...
            if hparam["candidate_mode"] == "random_one":
                candidates = CNJMolAssembler.enum_assemble(
                    node, neibs, prev_nodes=[], prev_amap=[], hparam=hparam
                )
                if len(candidates) > hparam["n_candidates"]:
                    candidates = CNJMolAssembler.get_target_candidates(
                        candidates,
                        n_candidates=hparam["n_candidates"],
                ...
                if len(node.candidates) == 0:
                    ...
                    for cand_sml in node.candidates:
                        ...
                        cands = CNJMolAssembler.enum_assemble(
                            node_copy, neibs, prev_nodes=[], prev_amap=[], hparam=hparam
                        )
            ...
            if len(leave_nb) > 1:
                raise ValueError(
                    "assemble_JTMolTree_degree_first get more node which input degree is more than one!"
                )
            else:
                idx = -1
                if len(candidates) > 0:
                ...
                if idx >= 0:
                    ...
                    print(
                        "assemble_one_node:Candidates is None! Ignore assemble info, keep node smiles"
                    )
                    if len(leave_nb) == 1:
                        uvisited[leave_nb[0].idx] = 1
                        uvisited[node.idx] = 0
    ...
    def assemble_JTMolTree_degree_first(moltree, uvisited, hparam=CNJASM_HParam()):
        len(uvisited)
        node_leaf = []
            ...
            while sum(uvisited) > 1:
                ...
                for node in node_leaf:
                    if CNJMolAssembler.can_be_assembled(node):
                        CNJMolAssembler.assemble_one_node(
                            node.neighbors[0], uvisited, hparam
                        )
    ...
    def assemble_order(moltree, use_stereo=False):
        ...
        cur_mol = CNJMolAssembler.dfs_assemble_without_score(
            tree_mess=tree_mess,
            mol_vec=mol_vec,
    ...
    def dfs_assemble_without_score(
        tree_mess,
        mol_vec,
        all_nodes,
        cur_mol,
        ...
        for i in range(len(cand_idx)):
            ...
            for nei_node in children:
                if nei_node.is_leaf:
                    continue
                cur_mol_b = CNJMolAssembler.dfs_assemble_without_score(
                    tree_mess=tree_mess,
                    mol_vec=mol_vec,
                    all_nodes=all_nodes,
                    cur_mol=cur_mol,
        ...
        try:
            ...
            if hparam["alg"] == "CALD_TSSA":
                ...
                if hparam["assemble_alg"] == "JTVAE_decode":
                    if (
                        hparam["n_candidates"] > 1
                    ):  # get more candidates for Goal-directed Reconstruction for TSSA,, set ncand < 20 in ChemUtils.enum_assemble()
                        final = []
                    ...
                    if final is None:
                        final = CNJMolAssembler.assemble_JTMolTree_degree_first(
                            moltree,
                            uvisited=uvisited,
                            hparam=CNJMolAssembler.get_hparam(asm_alg="CALG_TSDY"),
                        )
                    ...
                    final = CNJMolAssembler.assemble_JTMolTree_leaf_first(
                        moltree,
                        uvisited=uvisited,
                        hparam=CNJMolAssembler.get_hparam(asm_alg="CALG_TSDY"),
                    )

                cur_mol = CNJMolAssembler.get_final_mol(
                    final,
                ...
                final = CNJMolAssembler.assemble_JTMolTree_leaf_first(
                    moltree, uvisited=uvisited, hparam=hparam
                )
                ...
                cur_mol = CNJMolAssembler.assemble_Brics_bfs(
                    moltree,
                    uvisited=uvisited,
                )
            ...
            try:
                ...
                print(
                    "[[CNJMolAssembler.assemble_JTMolTree.select final].Exception",
                    e.args,
                )
                if isinstance(cur_mol, rdkit.Chem.rdchem.Mol):
    ...
    def dfs_assemble(
        all_nodes, cur_mol, global_amap, fa_amap, cur_node, fa_node, prob_decode
    ):
        fa_nid = fa_node.nid if fa_node is not None else -1
        prev_nodes = [fa_node] if fa_node is not None else []
        ...
        for i in range(len(cand_idx)):
            ...
            for nei_node in children:
                if nei_node.is_leaf:
                    continue
                cur_mol = CNJMolAssembler.dfs_assemble(
                    all_nodes,
                    cur_mol,
                    new_global_amap,
                    pred_amap,
    ...
    def Assemble_Mol(
        ...
        try:
            if hparam["alg"] == "CALG_TSIS":
                pred_root, pred_nodes, g, moltree = (
                    CNJMolAssembler.Convert_amt_JTMoltree(
                        vocab, ctoken, g, bfs_node_map, show
                    )
                )
            else:
                pred_root, pred_nodes, g, moltree = (
                    CNJMolAssembler.Convert_to_JTMoltree(
                        vocab, ctoken, bfs_binary_tree, bfs_node_map, show
                    )
                )

            dec_smile = CNJMolAssembler.assemble_JTMolTree(
                moltree,
                ctoken=ctoken,
                hparam=hparam,
            )
    ...
    def decode_single(
        ...
        try:
            ...
            for i, sub_s in enumerate(sub_smils):
                ...
                skt_wrong = 0
                if hparam["alg"] == "CALG_TSIS":
                    bfs_tree, g_amt, bfs_node_map = CNJTMolTree.bfs_ex_reconstruct_amt(
                        ctoken=ctoken, bfs_ex_smiles=bfs_ex_smiles, show=False
                    )  # bfs_tree is not linked
                else:
                    g_amt = None
                    bfs_tree, g_CNXBinaryTree, bfs_node_map, skt_wrong = (
                        CNJTMolTree.bfs_ex_reconstruct(
                            ctoken=ctoken, bfs_ex_smiles=bfs_ex_smiles, show=False
                        )
                    )  # bfs_tree is linked
                ...
                if bfs_tree is not None:
                    (
                        ...
                        new_voc,
                        bfs_ex_smarts,
                    ) = CNJTMolTree.get_bfs_ex(
                        ctoken, bfs_tree
                    )  # generate advanced bfs

                    if (
                        hparam["alg"] == "CALG_TSSA"
                        or hparam["alg"] == "CALG_TSDY"
                        or hparam["alg"] == "CALG_TSID"
                    ):
                    ...
                    for k in range(n_samples):
                        ...
                        dec_smile = CNJMolAssembler.Assemble_Mol(
                            vocab=None,  # vocab,
                            ctoken=ctoken,
...
def rebuild_file(n_samples=1):
    ...
    for i, bfs_ex_smiles in tqdm(
        enumerate(smiles_list), total=len(smiles_list), desc="parsing smiles ..."
    ):
        for k in range(n_samples):
            re_smils, bfs_ex_smiles_sub, new_vocs_sub, skt_wrong = (
                CNJMolAssembler.decode_single(
                    bfs_ex_smiles, ctoken, asm_alg, n_samples=1, p_mean=None
                )
            )
