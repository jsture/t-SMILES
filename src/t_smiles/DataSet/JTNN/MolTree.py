...
from t_smiles.MolUtils.RDKUtils.Frag.RDKFragBrics import RDKFragBrics
...
class MolTreeNode(object):
    def __init__(
        self, smiles, clique=[], smarts=None, kekuleSmiles=True, frg_random=False
    ):
        self.smiles = smiles
    ...
    def recover(self, original_mol):
        ...
        if not self.is_leaf:  # type: ignore
            for cidx in self.clique:
                original_mol.GetAtomWithIdx(cidx).SetAtomMapNum(self.nid)  # type: ignore
...
class MolTree(object):
    def __init__(
        ...
        try:
            ...
            self.atmomic_nums_dict = sorted(
                set([atom.GetAtomicNum() for atom in self.atoms] + [0])
            )
            ...
            self.bond_labels = [Chem.rdchem.BondType.ZERO] + list(
                sorted(set(bond.GetBondType() for bond in self.bonds))
            )
            ...
            for k in range(n_sub):
                ...
                if dec_alg == Fragment_Alg.Vanilla:
                    ...
                    s_cliques, s_edges, motif_str, dummy_atom, motif_smarts = (
                        RDKFragBricsDummy.decompose_dummy(s_mol)
                    )
                elif dec_alg == Fragment_Alg.MMPA_DY:
                    s_cliques, s_edges, motif_str, dummy_atom, motif_smarts = (
                        RDKFragMMPA.decompose_dummy(s_mol)
                    )
                elif dec_alg == Fragment_Alg.Scaffold_DY:
                    s_cliques, s_edges, motif_str, dummy_atom, motif_smarts = (
                        RDKFragScaffold.decompose_dummy(s_mol)
                    )
                elif dec_alg == Fragment_Alg.RBrics_DY:
                    s_cliques, s_edges, motif_str, dummy_atom, motif_smarts = (
                        RDKFragRBrics.decompose_dummy(s_mol)
                    )
                else:
                    s_cliques = [list(range(n_atoms))]
                    s_edges = []
            ...
            for i, c in enumerate(cliques):
                if dec_alg == Fragment_Alg.Vanilla:
                    ...
                    dec_alg == Fragment_Alg.BRICS_DY
                    or dec_alg == Fragment_Alg.MMPA_DY
                    or dec_alg == Fragment_Alg.Scaffold_DY
                    or dec_alg == Fragment_Alg.RBrics_DY
                ):
                    csmiles = motif_str[i]
                    csmarts = motif_smarts[i]
                else:
                    cmol, csmiles = ChemUtils.get_clique_mol(
                        mol=self.mol, atoms=c, kekuleSmiles=True
                    )
                    csmarts = csmiles
