...
class ChemUtils:
        ...
        try:
            ...
            if kekuleSmiles:
                ...
                if not isomericSmiles:
                    ...
                    sml = Chem.MolToSmiles(
                        new_mol, kekuleSmiles=True, isomericSmiles=True
                    )  # , rootedAtAtom = 0
            ...
            print(
                f"[ChemUtils.get_clique_mol].exception[kekuleSmiles={kekuleSmiles}]:",
                e.args,
            )
    ...
    def local_attach(ctr_mol, neighbors, prev_nodes, amap_list):
        ...
        ctr_mol = ChemUtils.attach_mols(ctr_mol, neighbors, prev_nodes, nei_amap)
    ...
    def enum_attach(ctr_mol, nei_node, amap, singletons):
        ...
        if nei_mol.GetNumBonds() == 0:
            ...
            for atom in ctr_atoms:
                if (
                    ChemUtils.atom_equal(atom, nei_atom)
                    and atom.GetIdx() not in used_list
                ):
    ...
    def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
        ...
        def search(cur_amap, depth):
            ...
            cand_amap = ChemUtils.enum_attach(node.mol, nei_node, cur_amap, singletons)
            cand_smiles = set()
            candidates = []
            for amap in cand_amap:
                cand_mol = ChemUtils.local_attach(
        ...
        for amap in all_attach_confs:
            cand_mol = ChemUtils.local_attach(node.mol, neighbors, prev_nodes, amap)
    ...
    def dfs_assemble(cur_mol, global_amap, fa_amap, cur_node, fa_node):
        ...
        for nei_node in children:
            if not nei_node.is_leaf:
                ChemUtils.dfs_assemble(
                    cur_mol, global_amap, label_amap, nei_node, cur_node
                )
