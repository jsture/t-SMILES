    ...
    def get_scaffold_frag(sml):
        ...
        for bond_idx in none_ring_bonds_list:
            ...
            if mol_in.GetBondWithIdx(bond_idx).GetBondTypeAsDouble() == 1.0:
                if (
                    mol_in.GetAtomWithIdx(bgn_atom_idx).IsInRing()
                    + mol_in.GetAtomWithIdx(ebd_atom_idx).IsInRing()
                    == 1
                ):
        ...
        try:
            if len(cut_bonds) > 0:
                ...
                cutted = Chem.FragmentOnBonds(
                    mol_in, cut_bonds, addDummies=True, dummyLabels=dummyLabels
                )
