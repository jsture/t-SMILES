...
from t_smiles.MolUtils.RDKUtils.Frag.RDKFragUtil import RDKFragUtil, JointPiece
from t_smiles.Tools.StringUtils import StringUtils
...
class RDKAssembling:
    ...
    def strip_dummy_atoms(mol):
        ...
        mols = Chem.ReplaceSubstructs(
            mol, RDKFragUtil.dummy_mol, hydrogen_mol, replaceAll=True
        )
        ...
        try:
            while itry < n_base:
                ...
                for i in range(len(neighbor.GetAtoms())):
                    new_mol = RDKAssembling.assemb_fragment(
                        base_mol, posi1, neighbor, i
                    )
                ...
                for value in base_dummys:
                    ...
                    for value_nb in nbr_dummys:
                        ...
                        if bnd_type_base == bnd_type_nb:
                            score = RDKFragUtil.score_atomenv(
                                atomenv_base,
                                atomenv_nbr,
                                RDKAssembling.get_atomenv_table(),
                                radius=3,
                            )
        ...
        try:
            ...
            if show:
                ...
                Draw.MolsToGridImage(
                    ...
                    molsPerRow=3,
                    subImgSize=(500, 500),
                    legends=[Chem.MolToSmiles(m, kekuleSmiles=False) for m in mols],
                ).show()
            ...
            if show:
                ...
                Draw.MolsToGridImage(
                    ...
                    subImgSize=(500, 500),
                    legends=[
                        Chem.MolToSmiles(m, kekuleSmiles=False) for m in [final_mol]
                    ],
                ).show()
            ...
            print(
                "[RDKAssembling.join_singel_frag].exception:",
                Chem.MolToSmiles(mol_base),
            )

        return final_mol

    def join_substructs(
        mol_base, neighbour, base_info: JointPiece, nb_info: JointPiece
    ):
        n_atoms_base = mol_base.GetNumAtoms()
        neighbour.GetNumAtoms()
        ...
        try:
            ...
            print(
                "[RDKAssembling.join_substructs].exception:", Chem.MolToSmiles(mol_base)
            )
            print(
                "[RDKAssembling.join_substructs].exception:",
                Chem.MolToSmiles(neighbour),
            )

        return final_mol

    def rdkit_ReplaceSubstructs(
        base_mol, neighbour, base_info: JointPiece, nb_info: JointPiece
    ):
        try:
            pt = nb_info.atom_idx
            ...
            try:
                ...
                print(
                    "[RDKAssembling.rdkit_ReplaceSubstructs.[Chem.ReplaceSubstructs]].exception:",
                    e.args,
                )
            ...
            print(
                "[RDKAssembling.rdkit_ReplaceSubstructs].base_mol:",
                Chem.MolToSmiles(base_mol),
            )
            print(
                "[RDKAssembling.rdkit_ReplaceSubstructs].neighbour:",
                Chem.MolToSmiles(neighbour),
            )
        ...
        if show:
            ...
            mols.extend(cands)
            Draw.MolsToGridImage(
                mols, subImgSize=(500, 500), legends=[Chem.MolToSmiles(m) for m in mols]
            ).show()
            ...
            try:
                ...
                if has_sub:
                    if replace_alg == "join_sub":
                        join_mol = RDKAssembling.join_substructs(
                            base_mol,
                            neighbour,
                            base_info,
                            nb_info,
                        ...
                        joined_mols = RDKAssembling.rdkit_ReplaceSubstructs(
                            base_mol, neighbour, base_info, nb_info
                        )
                ...
                print(
                    "[RDKAssembling.join_substructs-rdkit_ReplaceSubstructs].exception:",
                    e.args,
                )
                continue
    ...
    def assemb_mols_single(base_mol, match_alg="match_dummy_idx"):
        ...
        if match_alg != "match_dummy_idx":
            return [base_mol], [sml_base], [0.0]

        sgl_dummys = RDKFragUtil.get_dummy_info(base_mol)
        match_joints = RDKAssembling.get_matched_points(
            sgl_dummys, sgl_dummys, match_alg, dif_mol=False
        )
        ...
        try:
            ...
            if len(snd_dummy) == 2:
                ...
                join_mol = RDKAssembling.join_singel_frag(
                    base_mol,
                    base_info,
                    nb_info,
                )
    ...
    def assemb_mols_dummy(
        ...
        if match_alg == "match_dummy_idx" and not RDKFragUtil.cut_is_valid(
            [sml_base, sml_nbr]
        ):
            print("[match_dummy_idx] ids format dismatch, use match_all instead!")
        ...
        try:
            ...
            match_idxs = RDKAssembling.get_matched_points(
                base_dummys, nbr_dummys, match_alg, dif_mol=True
            )

            if len(match_idxs) == 0:
                print("[match_idxs].length is zero, use match_all instead")

                match_alg = ("match_all",)
                match_idxs = RDKAssembling.get_matched_points(
                    base_dummys, nbr_dummys, match_alg, dif_mol=True
                )
                print("[match_idxs].length:", len(match_idxs))
            ...
            if match_alg == "match_dummy_idx" and len(match_joints) == 2:
                ...
                cands_smls, cands_mols, scores = RDKAssembling.combine_frages_info(
                    base_mol, neighbour, match_joints_tmp, replace_alg
                )

                if len(cands_mols) > 0:
                    cands_mols_2nd, cands_sml_2nd, scores_2nd = (
                        RDKAssembling.assemb_mols_single(
                            cands_mols[0], match_alg="match_dummy_idx"
                        )
                    )
                ...
                cands_smls, cands_mols, scores = RDKAssembling.combine_frages_info(
                    base_mol, neighbour, match_joints, replace_alg
                )
...
def test_assemb_mols_dummy():
    ...
    Chem.MolToSmiles(base)
    Chem.MolToSmiles(neib)

    joined, scores = RDKAssembling.assemb_mols_dummy(
        base,
        neib,
        match_alg="match_dummy_idx",  # match with atom ids, only match one point
        # match_alg = 'match_atomenv',   #math with atom env
