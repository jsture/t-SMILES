...
class RDKFragUtil:
    ...
    def get_atomenv_table():
        ...
        for sf in s_file:
            ...
            df = pd.read_csv(
                sf, squeeze=True, delimiter=",", header=None, skip_blank_lines=True
            )
                ...
                for bond in dummy_donds:
                    if atom_idx in bond[0]:
                        for rds in range(0, radius):
                            smarts = RDKitUtils.getSmarts(
                                frag_mol, atomID, radius=rds, include_H=True
                            )
    ...
    def GetMolFrags(mol, break_bonds, addDummies=True):
        ...
        if addDummies:
            broken = Chem.FragmentOnBonds(
                mol, bondIndices=break_bonds, addDummies=True, dummyLabels=dummyLabels
            )
        else:
            broken = Chem.FragmentOnBonds(
                mol, bondIndices=break_bonds, addDummies=False
            )

        fragsMolAtomMapping = []
        try:
            frags_mol = Chem.GetMolFrags(
                broken, asMols=True, fragsMolAtomMapping=fragsMolAtomMapping
            )  # somtimes exception:('non-ring atom 1 marked aromatic',)
        except Exception as e:
        ...
        for fg in frags_mol:
            try:
                ...
                print(
                    "[RDKFragUtil.Chem.Kekulize(fg)].exception:can not Kekulize mole!"
                )
                raise Exception(e)
        ...
        try:
            ...
            if mol is not None:
                ...
                for rds in range(0, radius):  # radius shoudl be 1 based
                    ...
                    if label:
                        smarts = RDKUtils.getSmarts(
                            mol, atomID, radius=rds, include_H=True
                        )  # if radius>0: get AE
                    else:
            ...
            for atom in mol.GetAtoms():
                if atom.GetAtomicNum() == 0:
                    ...
                    for bond in bonds:
                        ...
                        bond.GetOtherAtom(atom).GetAtomicNum()
                        bond_type = bond.GetBondType()

                        joint_piece = RDKFragUtil.mol_add_atom(
                            base_mol=RDKFragUtil.dummy_mol,
                            insert_pos=0,
                            atom=nbr_atom,
                            bond_type=bond_type,
                        )
                        nbr_atom_env = RDKFragUtil.get_atomenv(
                            sml=Chem.MolToSmiles(mol), atomID=nbr_idx, radius=3
                        )
                        if atom_idx < nbr_idx:
