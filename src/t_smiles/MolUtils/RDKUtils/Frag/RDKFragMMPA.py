...
from t_smiles.MolUtils.RDKUtils.Frag.RDKFragUtil import RDKFragUtil
...
class RDKFragMMPA:
    def decompose_dummy(
        ...
        try:
            ...
            if len(break_bonds) > 0:
                frags_mol, frags_smarts, frags_sml, motifs_aidx, dummy_atoms = (
                    RDKFragUtil.GetMolFrags(mol, bond_idxs, addDummies=True)
                )
            ...
            if break_ex:
                for c in cliques:
                    if len(c) > 1:
                        ...
                        if break_long_link:
                            if (
                                not mol.GetAtomWithIdx(c[1]).IsInRing()
                                and not mol.GetAtomWithIdx(c[0]).IsInRing()
                            ):
                                breaks.append(c)
                        ...
                        if break_r_bridge:
                            if (
                                mol.GetAtomWithIdx(c[0]).IsInRing()
                                and mol.GetAtomWithIdx(c[1]).IsInRing()
                            ):
    ...
    def find_break_bonds(mol_in, maxCuts=1):
        ...
        fr = rdMMPA.FragmentMol(
            mol, maxCuts=maxCuts, resultsAsMols=True, maxCutBonds=n_bonds
        )
        for f in fr:
...
def test():
    ...
    fr_sml = rdMMPA.FragmentMol(
        mmm, maxCuts=maxCuts, resultsAsMols=False, maxCutBonds=50
    )
    fr_sml = set(fr_sml)
